...
class ImageViewController: UIViewController {
    ...
    func sendDemoApiRequest(_ url: URL, accessToken: String){
        ...
        let postDataTask = session.dataTask(with: request as URLRequest, completionHandler: {
            data, response, error in
            DispatchQueue.main.async{
                    
                if let httpResponse = response as? HTTPURLResponse {
                    do{
                        let jsonDictionaryOrArray = try JSONSerialization.jsonObject(with: data!, options: .mutableContainers) as! [AnyHashable: AnyObject]
                        ...
                        if let imageURL = jsonDictionaryOrArray["image"] as? String{
                            ...
                            print(jsonDictionaryOrArray["Error"] as AnyObject)
                            print("\(jsonDictionaryOrArray)")
                        }
                    } catch {
